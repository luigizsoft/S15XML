...
public class Principal {
	...
	static final int f_TipoElemento						=13; //Determina si es un Dispositivo (D), Servicio (S), Coste (C) o Fee (F)
	...
	private static int CtrlNumCosts;
			...
			while((linea = reader.readLine())!=null) {
				...
				if (!ID_Servicio.equals(Campos[f_POB_ID_Unico])||!aContratcID.equals(Campos[f_eventContractID].trim())||!POB_Code.equals(Campos[f_POB_CD_POB])){
					if (NuevoFichero==1){ // Si se decide generar un nuevo fichero
						...
						tdInception.setCostList(CostList);
				...
				if (Utiles.EsServicio(Campos[f_TipoElemento])){
					...
					System.out.println("Can't find element type (Service/Device/Cost)");
				}
				////////////////////
				/// FIN POBS ///////
				////////////////////
